...
public class ModuloExpression implements IExpression
{
    ...
    public ModuloExpression(String left, String right)
    {
        this.left = left;
        this.right = right;

        if (!isLiteral(left) || !isLiteral(right)) {
            throw new RuntimeException("ModuloExpression only supports literals");
        }
    }
    ...
    private Object determineValue() {
        ...
        if (lType != rType) {
            throw new RuntimeException("Cannot reduce modulo literals of different types");
        }
        ...
        switch (lType) {
            ...
            case String -> {
                throw new RuntimeException("Cannot reduce modulo strings");
            }
            case Char -> {
                throw new RuntimeException("Cannot reduce modulo chars");
            }
            case Boolean -> {
                throw new RuntimeException("Cannot reduce modulo booleans");
            }
            default -> throw new RuntimeException("Cannot reduce modulo literals of type '%s'".formatted(lType));
        }
    }
}
