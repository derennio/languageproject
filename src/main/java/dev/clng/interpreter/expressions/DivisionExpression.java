...
public class DivisionExpression implements IExpression
{
    ...
    public DivisionExpression(String left, String right)
    {
        this.left = left;
        this.right = right;

        if (!isLiteral(left) || !isLiteral(right)) {
            throw new RuntimeException("DivisionExpression only supports literals");
        }
    }
    ...
    private Object determineValue() {
        ...
        switch (lType) {
            ...
            case Char -> {
                throw new RuntimeException("Cannot divide chars");
            }
            case Boolean -> {
                throw new RuntimeException("Cannot divide booleans");
            }
            default -> throw new RuntimeException("Cannot divide literals of type '%s'".formatted(lType));
        }
    }
}
